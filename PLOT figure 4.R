...
for(i in 1:length(labv)){
  ...
  scenario=paste0("~/scenario_",lab,".RData")
...
bias_all_C<-sapply(1:length(labv),function(i)MethodC[[labv[i]]][,1]) 
...
load("~/scenario_interaction_pattern_oracle.RData") #6.2
...
# ------------------------------------------------------------------------------------- #
dev.new()
par(mar=c(0, 0.5, 1.5, 10), mfrow=c(5,1), oma = c(3, 3, 1, 1), xpd=F)
...
plot(1:24, rep(NA,24), xaxt = "n", yaxt = "n", ylim = c(0 ,6.1), ylab="" ,xlab="")
sapply((1:6), function(j){
  points(vall[j,], Val_C[j,], pch=7-j, cex=1.25, col=colv  )
})

title(expression(paste("Mortality gain (x " , 10^-2,")") ), cex.main=1.5)
...
title(expression(paste("Better treatment probability (x 100%)") ), line = 0.5, cex.main=1.5)


plot_alls2(best*100)
title(expression(paste("Best treatment probability (x 100%)") ), line = 0.5, cex.main=1.5)

plot_alls2(nearbest5*100)
title(expression(paste("Near best treatment probability (x 100%), ", kappa, "=5%") ), line = 0.5, cex.main=1.5)
...
title(expression(paste("Near best treatment probability (x 100%), ", kappa, "=10%") ), line = 0.5, cex.main=1.5)
xxx=vall[3,]+0.5
sapply(1:4,function(i)
{axis(side=1, at=xxx[i] ,labels = c("S5.1", "S5.2", "S6.1", "S6.2")[i], col.axis=colv[i], cex.axis=1.5)} )


dev.print(pdf, '~/estimand2_intv_25052021.pdf')
