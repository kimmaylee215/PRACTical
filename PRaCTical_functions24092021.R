# 24/9/2021 updated function identify_bestR in line 574
# 09/09/2021 set fit_sub to adjust for pattern
# use this on hpc

# 07/09/2021 created by Kim M Lee, King's College London
# This script contains data generating function and analysis methods.
# Please check note 3 for a potential issue when using method D

# function "fit_sub" is to be used in methods B1, B2 and B3
# current setting: adjusts for treatment comparison (see note 1 in the "fit_sub")

# function "methodB2" can print which extra patterns contribute data for one replication, 
# will not work with multiple replications. (see note 2 in "methodB2")
# current setting: not displaying this but fit the model using "fit_sub"
...
fit_sub<-function(sub_data){
  ...
  # additional dummy variable to indicate pairwise comparisons, to be used in note 1 below
  treatment_pattern_label<-paste0(sub_data[,2], "_", sub_data[,3])
  ...
  if(length(unique(pattern_var))==1){ # there is only one pattern
    ...
    # note 1: adjust for pairwise comparisons instead of subgroup
    #my.glm<-myTryCatch( glm(response~ treatment_ind+ treatment_pattern_label, family = "binomial" ) )
  }
...
methodB2<-function(alldata=Alldata){
  ...
  minimal<-function(i){
    ...
    fit_sub(data1)
    # note 2: hash the above line and use the following line to see which extra patterns contribute data
    # works only in one data replication and print out_B2 below
    # return(c(i, pat.used[-which(pat.used==i)]))
  }
...
fit_robustSE_D<-function(alldata=Alldata){
  ...
  # ------------------------- **note 3** ------------------------------------------ #
  # R version would convert the class of y to factor, if so, use the following line instead
